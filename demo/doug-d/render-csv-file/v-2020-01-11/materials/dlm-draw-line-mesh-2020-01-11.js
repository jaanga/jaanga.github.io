...
DLM.getMenu = function () {
	...
	window.addEventListener( "onfileparsecsv", DLM.renderLines, false );
...
<details>
	...
	<p title="line width: 1 to 20" >
		line width <output id=DLMoutLineWidth class=floatRight >2</output><br>
		<input type="range" id="DLMrngLineWidth" min=0.05 max=5 step=0.2 value=1 oninput=DLM.renderLines(CFR.contours); >
	</p>
...
DLM.letsDance = function() {
	...
	THR.scene.add( spotLight1, spotLight2, spotLight3 );
	THR.scene.add( lightHelper1, lightHelper2, lightHelper3 );
...
DLM.reset = function () {
	...
	THR.scene.remove(edges, boxHelper);
