...
GCO.getMessage = function () {
	...
	lins = GCO.contourLines.children.length;
...
GCO.findMatch = function (segment) {
	...
	const pairs0 = GCO.segmentPairs.filter(pair => { // finds current segment and two adjoining segments
		const match1 = GCO.getVertexMatch(segment[0], pair[0]);
		const match2 = GCO.getVertexMatch(segment[0], pair[1]);
		const match3 = GCO.getVertexMatch(segment[1], pair[0]);
		const match4 = GCO.getVertexMatch(segment[1], pair[1]);
	...
	const segmentNext = segmentsAdjoin.find(pair => {
		...
		const match1 = GCO.getVertexMatch( pair[0], polygonLast );

		const match2 = GCO.getVertexMatch( pair[1], polygonLast );
