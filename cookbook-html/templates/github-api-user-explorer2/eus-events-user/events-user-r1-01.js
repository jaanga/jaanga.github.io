...
// https://jaanga.github.io/terrain3/elevations-core3/elevations-view-r6.html 
	...
	EUS.requestGitHubAPIUserEvents = function( user ) {
		...
		var xhr;
		var events, event, txt, dates, actor, repos, repo, commit;
...
if( i === 0 ) console.log( '', event );
				...
				if ( !EUS.repos.includes( event.repo.name ) ) { EUS.repos.push( event.repo.name ); }
...
//				}
			...
			EUS.buildReport( user );

console.log( 'repos', EUS.type.issuesEvents );

		}

	}

	EUS.buildReport = function( user ) {

		txt = '<h1>' + user + ' summary of 100 most recent events </h1>';

		txt += 'time period covers ' + EUS.dates.slice( -1 )  + ' to ' + EUS.dates[ 0 ] + b;

		for ( var i = 0; i < EUS.repos.length; i++ ) {

			txt += '<h2>repo: ' + EUS.repos[ i ] + '</h2>';

		}


		txt += 'Push events: ' + EUS.type.pushEvents + b +
			'Issues events: ' + EUS.type.issuesEvents + b +
		'';
