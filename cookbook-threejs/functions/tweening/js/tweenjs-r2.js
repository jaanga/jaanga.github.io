...
// tweens
	...
	function tweenAllToLocation( index, output ) {
		...
		for ( var i = 0; i < objects.length; i++ ) {
			...
			var ms = oud.ms ? oud.ms : duration;
			var tween = obj.userData.tween ? obj.userData.tween : tween2location;
			tween( obj, oud );

			if ( obj.geometry ) { info.innerHTML += ( 1 + i ) + ' ' + obj.name + '-' + obj.geometry.type + ' ' + ms.toFixed() + 'ms<br>'; }

		}

		if ( camera.userData.places.length > 0 ) { tweenCamera( camera, camera.userData.places[ index ] ); }

		if ( output ) { output.value = index; }
